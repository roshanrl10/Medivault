...
exports.login = (req, res, next) => {
    passport.authenticate('local', { session: false }, (err, user, info) => {
        ...
        if (!user) {
            console.warn('[Login] Failed:', info ? info.message : 'Unknown reason');
            return res.status(400).json({ msg: info ? info.message : 'Login failed' });
        }
    ...
    if (verified) {
        ...
        jwt.sign(
            ...
            (err, token) => {
                ...
                logAction(user._id, user.email, 'MFA_SUCCESS', req);
                res.json({ msg: 'MFA Verified & Enabled', mfa_verified: true, user: payload.user });
            }
        );
    } else {
...
exports.changePassword = async (req, res) => {
    ...
    try {
        ...
        if (!isMatch) {
            await logAction(user.id, user.email, 'PASSWORD_CHANGE_FAILURE', req);
        ...
        await logAction(user.id, user.email, 'PASSWORD_CHANGE_SUCCESS', req);
